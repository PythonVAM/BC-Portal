...
- **Cost Out:** Lead step 6 per-CC (`renderRvByCC`, `s6co:<cc>`), step 7 per-SET-area (`coOutSection`, `s7co:<area>`), CI step 1 per-CC (`renderCiByCc`, `ci1co:<cc>`), CI steps 2/3/4 per-CC inside the aggregate (`renderCiCcAggregate`, `ciagg-co:<cc>` / `ciagg-fte:<cc>`), CI step 4 review (`renderCiReview`, `ci4co:<cc>`).
...
**Per-CC summary on the Cost In screens** (`renderCiCcAggregate(mode,elId)`): a compact per-CC matrix (Lead Step-7 style, `s6-table`) at the top of each Cost In screen — cost on step 2 (`ci-cc-agg-cost`), FTEs on step 3 (`ci-cc-agg-fte`), and **both** on the Review & submit screen step 4 (`renderCiReview`, unique ids `ci-cc-agg-cost-r4` / `ci-cc-agg-fte-r4` — distinct because every screen stays in the DOM, so `getElementById` would otherwise hit the step-2/3 element first). **Every** assigned CC appears (sorted Dept → CC). Each CC's **Cost Out / FTEs Out** row is the expand trigger (`onclick="toggleCoView('ciagg-co:<cc>'|'ciagg-fte:<cc>')"`): expanding reveals that CC's full flat detail inline (canonical `roUniIdHead`/`roUniIdCells` + `fyHeaderCells`/`fyDataCells`, so month expansion follows `s6Exp`). CCs with **more than one** Cost In Contributor additionally show a "… In (all)" row + a Net row; "… In (all)" is the **live** total = the current user's draft (`ciData`/`ciFteData`) + other contributors' submitted values (`ciResponses`, excluding self), re-rendered on entry-grid input, so the contributor can see whether each shared CC balances. The "… In (all)" row is **itself** an expand trigger (`toggleCoView('ciagg-coin:<cc>'|'ciagg-ftein:<cc>')`): expanding it inserts a per-contributor split as indented sub-rows — "You (draft)" plus every other contributor assigned to that CC with their submitted total (or a **Pending** dash until they submit). The sub-rows sum to the "… In (all)" total. This **replaced** both the old separate Cost Out / FTE Out summary blocks (`renderCiCoSummaryTable` / `renderCiFteCoSummaryTable`, scopes `ci2co:summary` / `ci3fte:summary`) **and** the standalone "Other Cost In contributors" section (`renderCiOthersTable` / `ciOthersCollapsed`), all deleted to save vertical space. (The single-CC mirror path stamps `sourceCc` on its seeded `ciData`/`ciFteData` so the aggregate can match those rows.)
