...
**Reuse & format**: the allocation math is `computeUvFteAlloc(ccs)` → `{ccCode:{cc,ccTotals,allocatedRows,…}}` and the render is `uvFteBlockHTML(ccAlloc,view)`, where `view` is the **same By SET Area / By cost centre / Detail granularity** as the FTEs-by-cost-centre table above it: **`'area'`** = one row per SET area (SET Area · FY); **`'cc'`** = one row per cost centre (MU · Cost centre · FY); **`'detail'`** = one row per Project × UV (MU · Cost centre · Project · UV · FY). Each ends in a **"Total — R&D cost centres"** row. On the **BC Lead's FTE screen** (step 6b, `buildStep6b` → `#lead-uv-ftes-table`) the block reads `ciAggView['fte-lead']`, so the FTE summary's toggle drives both tables (no separate expander; `_aggRerender('fte-lead')` rebuilds step 6b). The **Cost Out review** (step 5, `renderRvUvFtes`) has no toggle, so its Show rows expander (`rvUvFteExpanded`/`toggleRvUvFteRows`) maps to `'cc'` (collapsed) vs `'detail'` (expanded). The Lead reads the same `ccData`/`fteData` (synced from `submittedBC` by `openLeadView`).
