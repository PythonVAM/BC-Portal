...
**One-side summary** (`renderCiCcAggregate(mode,elId)`): a **single flat `rv-table`** per mode — Cost on the receiver's Cost screen (`ci-cc-agg-cost`, ci-step1) and FTEs on the FTEs screen (`ci-cc-agg-fte`, ci-step2). It shows **one row per (Many) cost centre** with that CC's FY totals (the Many side the One contributor is consolidating), sorted Dept → CC, plus a **"Total — all cost centres"** grand-total row. The FY25/FY26 headers are **month-expandable** (`fyHeaderCells`/`fyDataCells` + `toggleS6Fy`/`s6Exp`); per-CC totals are aggregated into monthly arrays (`aggRow`) so the totals month-expand too. Each CC row's label cell is the expand trigger (`onclick="toggleCoView('ciagg-co:<cc>'|'ciagg-fte:<cc>')"`): expanding inserts that CC's full flat detail as a `colspan` row beneath it (canonical `roUniIdHead`/`roUniIdCells`). Title authored as "Cost out / FTEs out by cost centre" so `dirSwapNodes` flips it when Cost In is the Many side. Intentionally just totals-by-CC + drill-down (no Out/In/Net matrix or per-contributor split). It **replaced** the old separate Cost Out / FTE Out summary blocks and the "Other Cost In contributors" section, all deleted.
