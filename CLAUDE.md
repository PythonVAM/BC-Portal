...
**Per-CC aggregate on the Cost In screens** (`renderCiCcAggregate(mode)`): for any source CC that has **more than one** Cost In Contributor, the Cost In screens show a per-CC Out/In/Net matrix (Lead Step-7 style, `s6-table`) at the top — cost on step 2 (`ci-cc-agg-cost`), FTEs on step 3 (`ci-cc-agg-fte`), and **both** on the Review & submit screen step 4 (`renderCiReview`, unique ids `ci-cc-agg-cost-r4` / `ci-cc-agg-fte-r4` — distinct because every screen stays in the DOM, so `getElementById` would otherwise hit the step-2/3 element first). "Cost In (all)" is the **live** total = the current user's draft (`ciData`/`ciFteData`) + other contributors' submitted values (`ciResponses`, excluding self), so the contributor can see whether the CC balances across everyone. It re-renders on entry-grid input. Single-contributor CCs are not shown. (The single-CC mirror path now stamps `sourceCc` on its seeded `ciData`/`ciFteData` so the aggregate can match those rows.)
