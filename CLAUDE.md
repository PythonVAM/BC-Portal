...
### 2.1b Multiple transfers, defined upfront (step 1)

The BC Lead defines **up to 5 transfers** at setup (step 1), each its own block with **one contact + a Cost Out / Cost In direction toggle next to the contact** (`plannedTransfers`, `addTransferBlock`/`removeTransferBlock`/`setTransferDir`/`setTransferPerson`/`renderTransferBlocks`, cap `MAX_TRANSFERS=5`). Saving (`saveAndNotifyCoPersons`) seeds `submittedBC.plannedTransfers` (`[{no,person,manySide}]`) + `totalTransfers`, and builds **`submittedBC.transfers[]`** — one self-contained transfer per planned block `{no,person,manySide,status,cc2Sel,ccData,fteData,coSubmissions,costInPersons,ciResponses,contributors}`. Each transfer is a full Many→One transfer that **balances on its own**.

**Transfers are CONCURRENT — all live at once.** Every transfer starts `awaiting-co`, so **all contributors are notified at setup** and work **in parallel, in any order** (this replaced the old sequential "Start next transfer"/`advanceToNextTransfer`/`archivedInstances` model — those are gone). `submittedBC.transfers[]` is the source of truth; the working globals (`ccData`/`fteData`/`cc2Sel`/`costInPersons`/`ciResponses`/`contribs`/`bcManySide`) **plus** the mirrored top-level `submittedBC` fields are a working **copy** of the currently-**loaded** transfer (`submittedBC.currentTransferNo`). **`loadTransfer(no)`** copies a transfer into that working copy; **`syncLoadedTransfer()`** writes it back (called at the end of `submitToLead`, `submitCostIn`, `submitToCiPersons`). This keeps the per-screen code (which reads the globals / top-level fields) unchanged while letting each role act on whichever transfer their task belongs to. Helpers: `getTransfers()/getTransfer(no)/loadedTransferNo()/transferNoForContributor(pid)`, `isTransferBalanced(t)/allTransfersBalanced()`, `bcOverallStatus()` (`'submitted-to-group'`|`'ready-to-group'`|`'in-progress'`).

**Per-transfer routing.** `getCostOutTasks()` returns one task **per transfer** whose Many-side contributor is the current user and who hasn't submitted it (`openCoTask(no)` loads that transfer). `getCiIncoming()` scans every transfer's `costInPersons` (`openCiFlow(no)` loads it). The Lead's dashboard shows a single BC row → **`openBcOverview()`** (new `bc-overview` screen, `renderBcOverview`): one row per transfer (contributor · direction · assigned Cost In person · status + action). Each row drills in via **`openTransferReview(no,step)`** (`loadTransfer(no)`+`openLeadView`). **Cost In assignment is per transfer.** On the **multi-transfer** Summary (step7) the assign block shows **one picker per transfer** (`transferAssignListHTML`/`assignTransferCiPerson(no,pid)`/`getTransferCiPerson(no)`/`clearTransferCiPerson(no)`/`wireTransferCiPickers`/`renderTransferCiDropdown`) — each writes straight into its own transfer's `costInPersons` (assigning = notifying, since `getCiIncoming` routes once set) and the submit box reflects the **overall** state (gated on `allTransfersBalanced()`). The **single-transfer** path keeps the one global picker (`onePersonPickerHTML`/`setOnePerson`/`getOnePersonAssigned`/`clearOnePerson`/`submitToCiPersons`). **`submitAllToGroup()`** (offered on the overview and step7) is gated on `allTransfersBalanced()` and locks **every** transfer to `submitted-to-group`. Legacy/test code that sets `contribs` directly still works (one transfer, all `contribs` as Many-side contributors).

**Lead review aggregates ALL transfers.** `bcTransfersData()` returns every entry of `submittedBC.transfers[]` as `{manySide,ccData,fteData,ciResponses,cc2Sel}` (the loaded transfer prefers the live module `ciResponses`); `bcCostRows()`/`bcFteRows()` classify each transfer's rows **Out vs In by its own direction** (manySide `out` → ccData is Out / ciResponses is In; `in` → reversed). Step 7's `costEntries`/`fteEntries` use these (direction-aware Out/In/Net pivot across all transfers). Step 6 (Cost) / step 6b (FTEs) read the **merged Many side** via `_aggSrc('cost-lead'/'fte-lead')` (`mergedManyCcData`/`mergedManyFte`/`mergedBcCc2Sel`) — single-transfer is unchanged. The step-7 pivot tables are marked `.s7-pivot` and **excluded from `dirSwapNodes`** (they're absolute Out/In, already direction-correct); the rest of step 7 still swaps. The step-6b UV-allocated FTEs block also aggregates across all transfers (merged Many side).
...
**Reuse & format**: the allocation math is `computeUvFteAlloc(ccs)` → `{ccCode:{cc,ccTotals,allocatedRows,…}}` (each allocated row carries `taCode`/`taName`/`product`/`projectName`/`uv` — the **Project → Product → Therapy Area** roll-up, derived via the `PROJECTS` lookup since R&D rows don't carry `productName`). The render is `uvFteBlockHTML(ccAlloc,scope)`, with its **OWN By TA / By UV / By cost centre / Detail toggle** (state `uvFteView[scope]`, default `'uv'`; `setUvFteView(scope,view)`), **independent** of the FTE summary's view toggle above it: **`'ta'`** = one row per Therapy Area (TA · FY); **`'uv'`** = one row per TA × UV (TA · UV · FY); **`'cc'`** = one row per cost centre (MU · Cost centre · FY); **`'detail'`** = one row per Project × UV (MU · Cost centre · **TA · Product** · Project · UV · FY). The TA column header is **"TA"** and cells use 4-letter abbreviations (`taAbbr` → Onco / Immu / CVRe) to avoid crowding. Each ends in a **"Total — R&D cost centres"** row, and the block renders its own header + toggle. `scope` is **`'lead'`** on the BC Lead's FTE screen (step 6b, `buildStep6b` → `#lead-uv-ftes-table`; `setUvFteView('lead',…)` rebuilds step 6b) and **`'co'`** on the Cost Out review (step 5, `renderRvUvFtes` → `#rv-uv-ftes-table`; the old Show rows expander was removed). The shared aggregate component itself takes a `scope`-resolved data source (`_aggSrc`/`_aggCcs`): `'-co'` scopes read the contributor's live module `ccData`/`fteData`/`cc2Sel`, `'-lead'` and receiver scopes read `submittedBC`. The Lead reads the same `ccData`/`fteData` (synced from `submittedBC` by `openLeadView`).
...
**Unified columns:** `(MU · Cost centre · Dept · Curr ·) GL account · TA · Product · Project · UV` + the FY columns. **TA** (Therapy Area, 4-letter abbrev via `taAbbr`) is **always derived/read-only** (`.cod-derived` greyed cell) — following Project → Product → TA (`rowProductTa`); for **R&D** rows **Product is also derived/greyed** (from the Project, since R&D rows don't store `productName`), for **SMM** Product is the entered value, and GL-only rows show N/A. This applies to **both** the read-only unified tables (`roUniIdHead`/`roUniIdCells`, TA filterable in the Detail view) **and** the editable Cost Out Detail entry grid (step 3b, `coDetailIdCells`). **MU** (Management Unit — the second org layer, `ccMU(cc)` → e.g. `7300 Oncology R&D`, `1100 Finance`) leads the identity group (it's the sticky-left column), then Cost centre · Dept, then **Curr** (the CC's currency, `gca(cc).cur`). All four are CC-level columns shown on multi-CC tables (`roUniIdHead(true,hasSub)`) and omitted together where the CC is already in a block/section header (`roUniIdHead(false,hasSub)`). The filter spec (`ciFilterColSpec`) mirrors this order. Columns a row's shape doesn't use render as a greyed **N/A** (`.cod-na`):
...
**Where applied:** Lead review Cost/FTEs (`buildStep6`/`buildStep6b` → `renderCiCcAggregate`), the **Cost Out contributor's step-5 review** (`buildReview` → `renderCiCcAggregate` scopes `cost-co`/`fte-co`, reading the contributor's live module data via `_aggSrc`/`_aggCcs` — `#rv-co-agg`/`#rv-fte-agg`; replaced the old `renderRvCostOut`/`renderRvFtes`/collapsible "Cost Out & FTEs Out summary" box). Step 5 passes **`opts.noDetail`** (it's a summary screen — line-level data is reachable by navigating Back/Edit), so its toggles are just By SET Area / By cost centre; the Lead/receiver screens keep Detail, CI step 1 (`renderCiByCc`), CI step 2 (`renderCiCcAggregate` per-CC summary, `renderCiTable`, `renderCiCoTable`, single-CC auto-populated view), CI step 3 (`renderCiCcAggregate` FTE summary — flat, FTEs not $m), CI per-CC review (`renderCiReview`), and step 7 (`coOutSection` / `ciInSection`, plus the aggregate Cost/FTE/Net matrix).
...
**Step 7 (Lead Summary) is two pivot tables** (`buildStep7`, `s7Pivot`) — the final screen of the Lead's Cost→FTEs→Summary review. Each table's **columns** are *Out by SET area* → *In by SET area* → a final **Net** column (Net = In − Out per row); **rows** are the chosen dimension for a single selected year. A universal **Year toggle** (`s7Year`, `setS7Year`, default `fy26`) governs both tables. **Cost** rows toggle **Dept ↔ Cost centre** (`s7CostRowBy`, `setS7CostRowBy`) and keep the USD/Local toggle (`s7Currency`); **FTEs** rows are **Location** only (no UV view on the Summary — the UV-allocated FTEs live on the Lead's FTEs screen, step 6b). Entry collectors `costEntries()`/`fteEntries()` map each CC → (rowKey, SET-area, value); cells use `fmtCostM`/`fmtFte`. There is **no By-SET-Area drill-down** — line-level detail is on the prior Cost (step6) / FTEs (step6b) screens. The balance check + actions (`status==='ci-submitted'` gate, Submit to Group / Add another transfer / assign person) are unchanged; the Net total also confirms balance (~0).
...
- **Don't edit, rename, or delete any `vN/index.html` snapshot** (`v2/` … `v10/`). They are frozen archives that colleagues may be linking to. All work goes in the root `transfer-hub-prototype.html`; new versions are made by *copying* the root file into a new `vN/` folder, never by editing an existing one. **Narrow exception (has happened once):** a snapshot cut only moments earlier, before a fix landed, may be refreshed *in place* from the root file — this was done for **v10** at the user's explicit request. This is not a precedent for editing a snapshot that has been circulating; once a version has been shared for any real length of time, cut a fresh `vN+1` instead.
