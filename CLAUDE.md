...
- **Lead step 6** (`renderRvByCC`, "Review Cost Out · Assign Cost In") no longer uses per-CC `coViewWrap` collapsibles — its Cost Out and FTE Out reference tables are now the shared **One-side summary** component (`renderCiCcAggregate`) under scopes **`cost-lead`** (`#s6-agg-cost`) and **`fte-lead`** (`#s6-agg-fte`), each with the By cost centre / Detail toggle + per-column Detail filters. The contributor status chips and the single Cost-In person-assign picker are kept around them.
...
**One-side summary** (`renderCiCcAggregate(mode,elId,opts)`): a **single flat `rv-table`** per mode — Cost on the receiver's Cost screen (`ci-cc-agg-cost`, ci-step1) and FTEs on the FTEs screen (`ci-cc-agg-fte`, ci-step2), and **also the Lead's step 6** Cost/FTE reference tables (`#s6-agg-cost`/`#s6-agg-fte`, scopes `cost-lead`/`fte-lead`). `opts` carries a `scope` (state key, defaults to `mode`) and an explicit `ccs` list (the Lead passes the submitted CO/FTE CCs; the receiver defaults to its assigned CCs); all view/filter state and onclick handlers are keyed off `scope`. A **"By cost centre" / "Detail" toggle** (`ciAggView[scope]`, `setCiAggView(scope,level)`) switches the whole table between two internally-consistent views (no nested/inline expansion, which made columns misalign): **CC level** = one row per (Many) cost centre with FY totals; **Detail level** = one flat unified line-level table across all CCs (cost: `roUniIdHead(true)`/`roUniIdCells(r,true,'out')` → Cost centre·Dept·GL·Product·Project·UV; FTEs: Cost centre + location columns). Both views end in a **"Total — all cost centres"** row and have **month-expandable** FY25/FY26 headers (`fyHeaderCells`/`fyDataCells` + `toggleS6Fy`/`s6Exp`); per-CC/grand totals aggregate into monthly arrays via `aggRow` so they month-expand too. Title authored as "Cost out / FTEs out by cost centre" so `dirSwapNodes` flips it when Cost In is the Many side. It **replaced** the old separate Cost Out / FTE Out summary blocks and the "Other Cost In contributors" section, all deleted.
