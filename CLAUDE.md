...
The BC Lead defines **up to 5 transfers** at setup (step 1), each its own block with **one contact + a Cost Out / Cost In direction toggle next to the contact** (`plannedTransfers`, `addTransferBlock`/`removeTransferBlock`/`setTransferDir`/`setTransferPerson`/`renderTransferBlocks`, cap `MAX_TRANSFERS=5`). Saving (`saveAndNotifyCoPersons`) seeds `submittedBC.plannedTransfers` (`[{no,person,manySide}]`) + `totalTransfers` + `currentTransferNo=1`, and activates transfer 1 (its contact → `contributors`, its direction → `manySide`). Each transfer is a full Many→One transfer that **balances on its own**. They process **sequentially**: once the active transfer is `ci-submitted` & balanced, the Summary offers **"Start next transfer"** (`advanceToNextTransfer` — archives the active one to `archivedInstances`, activates the next planned transfer's contact+direction), and only **"Submit all to Group"** once every planned transfer is balanced. Legacy/test code that sets `contribs` directly still works (one transfer, all `contribs` as Many-side contributors).

**Lead review aggregates ALL transfers** (archived + active). `bcTransfersData()` returns every transfer as `{manySide,ccData,fteData,ciResponses,cc2Sel}`; `bcCostRows()`/`bcFteRows()` classify each transfer's rows **Out vs In by its own direction** (manySide `out` → ccData is Out / ciResponses is In; `in` → reversed). Step 7's `costEntries`/`fteEntries` use these (direction-aware Out/In/Net pivot across all transfers). Step 6 (Cost) / step 6b (FTEs) read the **merged Many side** via `_aggSrc('cost-lead'/'fte-lead')` (`mergedManyCcData`/`mergedManyFte`/`mergedBcCc2Sel`) — single-transfer is unchanged. The step-7 pivot tables are marked `.s7-pivot` and **excluded from `dirSwapNodes`** (they're absolute Out/In, already direction-correct); the rest of step 7 still swaps. The step-6b UV block reflects the **active** transfer only.
