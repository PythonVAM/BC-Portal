...
**One-side summary on the Cost In screens** (`renderCiCcAggregate(mode,elId)`): a **single flat table** per mode (`s6-table`) at the top of each Cost In screen — cost on step 2 (`ci-cc-agg-cost`), FTEs on step 3 (`ci-cc-agg-fte`), and **both** on the Review & submit screen step 4 (`renderCiReview`, unique ids `ci-cc-agg-cost-r4` / `ci-cc-agg-fte-r4` — distinct because every screen stays in the DOM, so `getElementById` would otherwise hit the step-2/3 element first). It shows **one row per (Many) cost centre** with that CC's FY totals (the Many side the One contributor is consolidating), sorted Dept → CC, plus a **"Total — all cost centres"** grand-total row. Each CC row's label cell is the expand trigger (`onclick="toggleCoView('ciagg-co:<cc>'|'ciagg-fte:<cc>')"`): expanding inserts that CC's full flat detail as a `colspan` row beneath it (canonical `roUniIdHead`/`roUniIdCells` + `fyHeaderCells`/`fyDataCells`, so month expansion follows `s6Exp`). Title authored as "Cost out / FTEs out by cost centre" so `dirSwapNodes` flips it when Cost In is the Many side. This is intentionally just totals-by-CC + drill-down — the Many→One model has a single One-side contributor and one One CC, so the old per-CC Out/In/Net matrix and per-contributor split were dropped (the balance is enforced by `submitCostIn`'s gate). It **replaced** the old separate Cost Out / FTE Out summary blocks (`renderCiCoSummaryTable` / `renderCiFteCoSummaryTable`) and the standalone "Other Cost In contributors" section (`renderCiOthersTable`), all deleted. (The single-CC mirror path stamps `sourceCc` on its seeded `ciData`/`ciFteData`.)
