...
**Reuse & format**: the allocation math is `computeUvFteAlloc(ccs)` → `{ccCode:{cc,ccTotals,allocatedRows,…}}` and the render is `uvFteBlockHTML(ccAlloc,expanded)`. The block is a **single flat table standardised to match the FTEs-by-cost-centre table above it**: lead columns **MU · Cost centre**, then the UV-specific **Project · UV**, then the FY columns + a **"Total — R&D cost centres"** grand-total row. Collapsed = just the total row; expanded = one row per (CC × Project × UV). Both the Cost Out review (step 5, `renderRvUvFtes`, `rvUvFteExpanded`/`toggleRvUvFteRows`) and the **BC Lead's FTE screen** (step 6b, `buildStep6b` → `#lead-uv-ftes-table`, `leadUvFteExpanded`/`toggleLeadUvFtes`) render from them. The Lead reads the same `ccData`/`fteData` (synced from `submittedBC` by `openLeadView`).
